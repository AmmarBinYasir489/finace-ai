...
def add_content(doc: Document) -> None:
    ...
    file_rows = [
        ...
        ["voicetrack/constants.py", "Stores fixed categories, transaction types, and dark/light theme colors."],
        ...
        ["voicetrack/dates.py", "Converts words like yesterday or last week into real dates such as 2026-06-27."],
        ["voicetrack/extractors.py", "Talks to Ollama and parses JSON. If Ollama fails, it uses a simple local fallback parser."],
        ["voicetrack/pipeline.py", "Validates extracted data, fills missing date/time, normalizes category, and saves safe transactions."],
        ["voicetrack/speech.py", "Captures microphone audio with sounddevice and converts speech to text using Vosk."],
        ...
        ["tests/test_pipeline.py", "Automated tests that check the main transaction flow without needing Ollama or a microphone."],
    ...
    for step in [
        ...
        "User clicks Process.",
        "The UI sends the text to TransactionPipeline.preview().",
        "The pipeline asks the extractor to understand the sentence.",
        "Ollama returns JSON. If Ollama is too slow or fails, the local fallback parser tries simple extraction.",
        "The pipeline checks the data: amount must exist, type must be income or expense, category must be valid, and date/time are normalized.",
        "The preview appears so the user can review or edit fields.",
        "User clicks Confirm.",
